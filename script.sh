...
for i in carlos maria joao; do
  useradd $i -m -s /bin/bash -p $(openssl passwd -1 Senha123) -G GRP_ADM
done

for i in debora sebastiana roberto; do
  useradd $i -m -s /bin/bash -p $(openssl passwd -1 Senha123) -G GRP_VEN
done

for i in josefina amanda rogerio; do
  useradd $i -m -s /bin/bash -p $(openssl passwd -1 Senha123) -G GRP_SEC
done
